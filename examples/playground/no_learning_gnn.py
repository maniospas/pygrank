...
graph, features, groups = pg.load_feature_dataset(
    "citeseer", groups_no_labels=True, graph_api=nx
)
nodes = list(graph)
# graph = nx.Graph()
for node in nodes:
...
for seed in range(100):
    ...
    ranks_set = [
        ranker(graph, {node: 1 for node in group if node in training})
    ...
    tp = 0
    for v in test:
        if max(options, key=lambda i: ranks_set[i][v]) == max(
            options, key=lambda i: groups[i][v]
        ):
            tp += 1
