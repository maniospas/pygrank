        ...
        lambda params: pg.Normalize(
            postprocessor(
                pg.GenericGraphFilter(
                    ...
                    error_type="iters",
                    max_iters=41,
                    optimization_dict=pg.SelfClearDict(),
                    preserve_norm=False,
                )
            )
        ),
...
tuned = {
    "select": pg.AlgorithmSelection(
        pg.create_many_filters().values(),
        measure=lambda *args: pg.Conductance(autofix=True),
...
pg.benchmark_print(
    pg.benchmark_average(
        pg.benchmark(algorithms | tuned, loader, measure, fraction_of_training=0.3)
    ),
    decimals=3,
    delimiter=" & ",
    end_line="\\\\",
