...
class IdentitySerializer(tf.keras.initializers.Initializer):
    def __call__(self, shape, dtype=None, **kwargs):
        return tf.concat(
            [
                tf.constant(1, shape=(1, shape[1]), dtype=dtype),
                tf.zeros(shape=(shape[0] - 1, shape[1]), dtype=dtype),
            ],
            axis=0,
        )
...
class Tensortune(pg.Postprocessor):
        ...
        for _ in range(self.depth):
            model.add(
                tf.keras.layers.Dense(
                    dims, kernel_initializer=IdentitySerializer(), activation="relu"
                )
            )
    ...
    def train_model(self, graph, personalization, sensitive, *args, **kwargs):
        original_ranks = self.postprocessor(self.ranker)(
            graph, personalization, *args, **kwargs
        )
        ordered = [
            original_ranks.get(v)
            for v in sorted(original_ranks, key=original_ranks.get, reverse=True)
        ]
        self.min_diff = min(
            [
                abs(ordered[i] - ordered[i + 1])
                for i in range(len(ordered) - 1)
                if abs(ordered[i] - ordered[i + 1]) > 0
            ]
        )
        prev_convergence = self.ranker.convergence
        self.ranker.convergence = pg.ConvergenceManager(
        ...
        exclude = (
            ...
            else tf.cast(personalization.np, tf.float32)
        )
        if self.zero_mabs is None:
            training_objective = (
                pg.AM(differentiable=False)
                .add(
                    pg.MSQRT(tf.cast(original_ranks.np, tf.float32), exclude=exclude),
                    weight=1,
                )
                .add(
                    pg.pRule(tf.cast(sensitive.np, tf.float32), exclude=exclude),
                    max_val=self.max_fairness,
                    weight=-self.fairness_weight,
                )
            )
        else:
            training_objective = (
                ...
                .add(
                    pg.Mabs(tf.zeros(original_ranks.np.shape, tf.float32)),
                    weight=float(self.zero_mabs),
                )
                .add(pg.Mabs(tf.cast(original_ranks.np, tf.float32), exclude), weight=1)
                .add(
                    pg.pRule(tf.cast(sensitive.np, tf.float32), exclude),
                    max_val=self.max_fairness,
                    weight=-self.fairness_weight,
                )
            )
            ...
            for epoch in range(10000):
                ...
                if validation_loss <= best_loss:  # -prev_convergence.tol:
                    ...
                    print(
                        ...
                        float(
                            pg.Mabs(
                                tf.cast(original_ranks.np, tf.float32), exclude=exclude
                            )(ranks)
                        ),
                        "prule",
                        float(
                            pg.pRule(
                                tf.cast(sensitive.np, tf.float32), exclude=exclude
                            )(ranks)
                        ),
                        end="",
                    )
