import pygrank as pg


datasets = ["CiteSeer", "EUCore"]
pre = pg.preprocessor(assume_immutability=True, normalization="symmetric")  # shared
algs = {
    "ppr.85": pg.PageRank(0.85, preprocessor=pre),
    "ppr.99": pg.PageRank(0.99, preprocessor=pre, max_iters=1000),
    "hk3": pg.HeatKernel(3, preprocessor=pre),
    "hk5": pg.HeatKernel(5, preprocessor=pre),
    "tuned": pg.ParameterTuner(preprocessor=pre),
}
loader = pg.load_datasets_one_community(datasets)
pg.benchmark_print(pg.benchmark(algs, loader, pg.AUC, fraction_of_training=0.5))
