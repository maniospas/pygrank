...
G, groups = import_SNAP_data(
    dataset, specific_ids=specific_ids
)  # left one is amazon, right is dblp
pre = preprocessor(
    "col", assume_immutability=True
)  # a preprocessor that hashes the outcome of normalization for faster running time of the same algoriths
...
for group_number in range(len(groups)):
    for alpha in [0.85, 0.90, 0.95, 0.99, 0.995, 0.999]:
        ...
        ground_truth_ranker = PageRank(
            alpha=alpha, to_scipy=pre, tol=1.0e-20, max_iters=30000, use_quotient=False
        )
        ground_truth_ranks = ground_truth_ranker.rank(G, seeds)
        result_iterations += " & " + str(ground_truth_ranker.convergence.iteration)
        print(
            "Found ground truth ranks ("
            + str(ground_truth_ranker.convergence.iteration)
            + " iterations)"
        )
        compared_rankers = list()
        for tol in [1.0e-6, 1.0e-7, 1.0e-8, 1.0e-9, 1.0e-10, 1.0e-11, 1.0e-12]:
            compared_rankers.append(
                PageRank(
                    alpha=alpha,
                    to_scipy=pre,
                    tol=tol,
        ...
        compared_rankers.append(
            PageRank(
                ...
                to_scipy=pre,
                tol=tol,
                max_iters=estimate_mixing(alpha),
                error_type="iters",
            )
        )
        compared_rankers.append(
            PageRank(
                ...
                to_scipy=pre,
                use_quotient=False,
                convergence=RankOrderConvergenceManager(
                    alpha, confidence=0.99, criterion="fraction_of_walks"
                ),
            )
        )
        compared_rankers.append(
            PageRank(
                ...
                use_quotient=False,
                convergence=RankOrderConvergenceManager(
                    alpha, confidence=0.98, criterion="rank_gap"
                ),
            )
        )
        for ranker in compared_rankers:
            ...
            # print(sp[0])
            result_spearmans += " & " + str(-int(np.log10(1 - sp[0]) * 10) / 10.0)
