...
class HopTuner(Tuner):
    ...
    def __init__(
        ...
        tunable_offset: Optional[
            Callable[[GraphSignal, GraphSignal], Supervised]
        ] = None,
    ...
    def _tune(self, graph=None, personalization=None, *args, **kwargs):
        ...
        measures = [
            self.measure(validation, training1),
        ...
        if self.basis == "krylov":
            ...
            basis = [
                arnoldi_iteration(M, p, len(measure_values))[0] for p in propagated
            ]
            for i in range(len(measure_values)):
                measure_values[i] = [
                    float(measure(base[:, i])) for base, measure in zip(basis, measures)
                ]
        ...
        if self.autoregression != 0:
            ...
            window = backend.repeat(1.0 / self.autoregression, self.autoregression)
            beta1 = 0.9
            beta2 = 0.999
            beta1t = 1
            beta2t = 1
            ...
            while True:
                ...
                for i in range(len(measure_values) - len(window) - 2, -1, -1):
                    parameters[i, :] = backend.dot(
                        (window), measure_values[(i + 1) : (i + len(window) + 1), :]
                    )
                errors = (
                    (parameters - measure_values)
                    * measure_weights
                    / backend.sum(measure_weights)
                )
                for j in range(len(window)):
                    gradient = 0
                    for i in range(len(measure_values) - len(window) - 1):
                        gradient += backend.dot(
                            measure_values[i + j + 1, :], errors[i, :]
                        )
                    momentum[j] = (
                        beta1 * momentum[j] + (1 - beta1) * gradient
                    )  # *np.sign(window[j])
                    ...
                    window[j] -= (
                        ...
                        * momentum[j]
                        / (1 - beta1t)
                        / ((rms[j] / (1 - beta2t)) ** 0.5 + 1.0e-8)
                    )
                    # window[j] -= 0.01*gradient*np.sign(window[j])
                error = backend.mean(backend.abs(errors))
                if error == 0 or abs(error - prev_error) / error < 1.0e-6:
                    best_parameters = parameters
                    break
        best_parameters = backend.mean(
            best_parameters[: self.num_parameters, :]
            * backend.to_primitive(measure_weights),
            axis=1,
        ) + backend.mean(mean_value)
        ...
        if self.tunable_offset is not None:
            ...
            best_offset = optimize(
                lambda params: -measure.best_direction()
                * measure(
                    self._run(
                        training,
                        [
                            best_parameters[i] + params[0]
            ...
            best_parameters = [
                best_parameters[i] + best_offset[0] for i in range(len(best_parameters))
            ]
        ...
        if self.basis != "krylov":
            return Tautology(), self._run(
                personalization, best_parameters, *args, **kwargs
            )  # TODO: make this unecessary
        return self.ranker_generator(best_parameters), personalization

    def _run(
        self, personalization: GraphSignal, params: object, base=None, *args, **kwargs
    ):
